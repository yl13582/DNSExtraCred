...
def dns_query(type, name, server):
    ...
    qname_parts = name.lower().split('.') # How can we easily split the string?
    qname_encoded_parts = [
        struct.pack('B', len(part)) + part.encode('ascii')
        for part in qname_parts
    ] # Make sure it's encoded as a sequence of the right character encoding type (lowercase)
    ...
    response_question = data[12:12+len(question)] # The data variable starts immediately after the header section
    ...
    for _ in range(ANCOUNT):
        ...
        type, cls, ttl, rdlength = struct.unpack('!HHIH', response_answer[offset:offset+10]) # What is the offset value in bytes? Remember 'H' represent 2 bytes, and 'I' represents 4 bytes.
